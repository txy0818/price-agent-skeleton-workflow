...
按 `[S4]` 展示。以下模板即初始化提案允许输出的完整范围；所有尖括号字段必须替换成本轮真实上下文、查询或 S3 返回值，不得原样输出占位符：
...
全文必须放入同一个四反引号 `text` 围栏，全文内部的三反引号代码围栏原样保留。`diff_record_id=0` 也如实展示，并按 `[S5]` 的完整 `prompt_content` 兜底路径处理；不得为了获取非零 ID 重复校验。
