...
[4.5 候选提示词格式模板（当前规则同步 + 格式重构硬分支）] 节。
...
1. **成功终态**：本轮校验成功且 `diffRecordId>0`，只输出第 5 节的完整 EDIT 提案模板。
...
`diffRecordId` 和 `diffContent` 重组第 5 节模板；任何过程草稿都必须丢弃，禁止保留在模板之前或之后。
...
1. `### 4.5 候选提示词格式模板（当前规则同步 + 格式重构硬分支）`；
2. `### 4.6 调用校验前的必检项`、`### 4.7 硬门禁`和 `### 4.8 调用校验`；
3. `## 5. 展示提案`及其完整“输出硬校验”；
...
2. **当前规则同步分支**：用户要求"根据/按照/依据/基于/通过/同步当前或最新的比价项规则、母子品牌关系、
   ...
   - `material=true`：用户明确提到"材质关系""材质映射""材质对照表"或等价表达；按本轮真实材质比价项
     同步材质引用、材质对照表和材质专属字段规则。真实 `compareItem` 均不含"材质"时，同步结果固定为删除
...
按 4.5 节格式模板完整审计本轮 `tool_query_prompt_skeleton` 响应中的
...
候选全文是内部工具输入，不等于允许向用户展示。从 4.1 开始按分支需要调用工具：需要类目规则才查
...
- **精确修改直达**
  - 必须执行：4.0.1 精确修改直达局部编辑规则。
  - 禁止执行：4.1 按需加载规则/关系结果；4.2 比价项转换账本；4.3 三类特殊比价项落点规则；4.5 候选提示词格式模板。
  - 4.6 调用校验前的必检项：第 1、3、4、8 条。

- **当前规则同步：`priceRules=true`**
  - 必须执行：4.1.1 类目规则查询；4.1.2 统一作用域过滤；4.2 比价项转换账本；4.3.3 货号；4.4 映射与比价项编号规则；4.5 候选提示词格式模板。
  - 追加执行：`brand=true` 时必须执行 4.3.1 品牌，且仅当品牌作用域非空时追加 4.1.3 主子品牌关系查询，
    空集时执行删除同步；`material=true` 时必须执行
    4.3.2 材质，且仅当材质作用域非空时追加 4.1.4 同款材质关系查询，空集时执行删除同步。
  - 禁止执行：未明确指定 `brand=true` 时禁止 4.1.3、4.3.1；未明确指定 `material=true` 时禁止 4.1.4、4.3.2。
  - 4.6 调用校验前的必检项：第 1、2、3、4、5、6、7 条。

- **当前规则同步：`priceRules=false && brand=true && material=false`**
  - 必须执行：4.1.1 类目规则查询；4.1.2 统一作用域过滤；4.3.1 品牌；4.4 映射与比价项编号规则。
  - 条件执行：`brandScopeCateNameTreeSet` 非空才执行 4.1.3；空集时禁止查询并执行删除同步。
  - 禁止执行：4.2 比价项转换账本；4.1.4 同款材质关系查询；4.3.2 材质；4.3.3 货号；4.5 候选提示词格式模板。
  - 4.6 调用校验前的必检项：第 4、5、6、7 条。

- **当前规则同步：`priceRules=false && brand=false && material=true`**
  - 必须执行：4.1.1 类目规则查询；4.1.2 统一作用域过滤；4.3.2 材质；4.4 映射与比价项编号规则。
  - 条件执行：`materialScopeCateNameTreeSet` 非空才执行 4.1.4；空集时禁止查询并执行删除同步。
  - 禁止执行：4.2 比价项转换账本；4.1.3 主子品牌关系查询；4.3.1 品牌；4.3.3 货号；4.5 候选提示词格式模板。
  - 4.6 调用校验前的必检项：第 4、5、6、7 条。

- **当前规则同步：`priceRules=false && brand=true && material=true`**
  - 必须执行：4.1.1 类目规则查询；4.1.2 统一作用域过滤；4.3.1 品牌；4.3.2 材质；4.4 映射与比价项编号规则。
  - 条件执行：`brandScopeCateNameTreeSet` 非空才执行 4.1.3；空集时禁止查询并执行删除同步。
  - 条件执行：`materialScopeCateNameTreeSet` 非空才执行 4.1.4；空集时禁止查询并执行删除同步。
  - 禁止执行：4.2 比价项转换账本；4.3.3 货号；4.5 候选提示词格式模板。
  - 4.6 调用校验前的必检项：第 4、5、6、7 条。

- **格式重构硬分支**
  - 必须执行：4.1.1 类目规则查询；4.1.2 统一作用域过滤；4.2 比价项转换账本；4.3.3 货号；4.4 映射与比价项编号规则；4.5 候选提示词格式模板。
  - 追加执行：按真实比价项门禁追加 4.1.3 主子品牌关系查询、4.3.1 品牌，或 4.1.4 同款材质关系查询、4.3.2 材质。
  - 禁止执行：无。
  - 4.6 调用校验前的必检项：全部。

- **无方向普通修改**
  - 必须执行：4.0.2 无方向普通修改整理规则。
  - 禁止执行：4.1 按需加载规则/关系结果；4.2 比价项转换账本；4.3 三类特殊比价项落点规则；4.5 候选提示词格式模板。
  - 4.6 调用校验前的必检项：第 1、3、4 条。
...
3. **机械一致性**：局部编辑后必须保持章节标题、JSON 字段规则、映射表格式、材质引用行与材质表对应关系、
   以及比价项章节和品牌映射编号连续；删除任一编号项后按 4.4 重新编号。
...
2. **禁止新增业务规则**：不得新增、删除或改写比价项；不得新增品牌互认、材质互认、货号例外、来源优先级、
   匹配逻辑、综合判定例外或类目适用范围。
3. **保持正文结构**：不得套用 4.5 候选提示词格式模板重建全文；基础正文已有固定章节时保持标题、顺序和主体内容。
...
3. 对每次 `radar_query_price_rule` 响应，读取 `data.labelCateRule` 并建立该 categoryId 的规则记录：
   ...
   查询完成后，后续 4.1.2、4.2、4.5 只能使用本轮 categoryId 规则记录，
...
2. 将每条有效原文分别与 `materialScopeLedger` 的每一行做联合相关性判断，不得先仅按 `cateNameTree`
   ...
   每个 `(materialText, materialScopeLedger行)` 初始必须为“不命中”，只能逐行证明后改为“命中”。
   禁止把 `materialScopeLedger` 全部行直接复制为每条 `materialText` 的作用域；无法明确区分是否适用于
   `主材质`、`配件材质`、`表带材质`、`表壳材质` 等具体维度时，不得记录到该维度；最终无任何明确作用域则删除该原文。
...
必须在 `materialRetentionLedger` 全部构建完成后再一次性渲染材质表，禁止“处理一条材质就立即输出一个标题”。
渲染算法固定为：
1. 设 `previousScopeLabels=空`，按账本原始相对顺序遍历每条保留原文；
2. 仅当当前 `scopeLabels != previousScopeLabels` 时输出一次 `#### 适用范围：<scopeLabels>`，然后更新 `previousScopeLabels`；
3. 无论是否输出标题，当前 `materialText` 都只输出一次；
4. 标题必须独占一行，标题末尾与第一条材质原文之间必须有换行；每条 `materialText` 也必须独占一行。
...
<materialText-2 原文>
```
禁止输出 `#### 适用范围：<scopeLabels><materialText>`，也禁止在两条连续同作用域原文之间重复该标题。
...
本轮查询成功但 `data.materialText[]` 为空，或联合过滤后 0 行时，锁定 `materialGroupCount=0` 并静默省略
材质引用和材质表，不得输出空结果说明，也不得停止。当前规则同步分支 `material=true` 时，这不是
“保持基础正文原样”：候选必须删除基础正文已有的全部材质对照表、所有 `- **材质对照表**` 引用行及
删除后孤立的分隔线；真实材质比价项及材质专属字段规则仍按当前类目规则保留，不得因映射零结果删除。
进入下一步前记录 `materialGroupCount = min(materialRetentionLedger 中去重原文行数, 50)`。
同时按上述公式记录 `materialScopeRunCount`。
仅当 `materialGroupCount>0` 时，过滤结果才用于生成材质对照表；按原始相对顺序遍历材质原文，
仅合并相邻且命中作用域集合完全相同的连续行，不得全局聚合或重排。
...
材质对照表、材质专属字段规则及其相邻的必要结构；
对应值为 `false` 的映射内容必须保持基础正文。

#### 4.2.1 来源过滤（生成 expectedPriority）
...
生成时将下方普通 Step3 和/或货号 Step3 直接填充到 4.5 完整格式模板的 Step3 指定位置。
...
#### 4.3.1 品牌
- 母子品牌关系的查询、过滤和去重只按 4.1.2、4.1.3 执行；本节只规定候选正文落点。
- `brandGroupCount>0` 时，只能输出到 `### 母子品牌映射表（<真实适用类目>）`；`brandGroupCount=0`
  时候选中不得出现母子品牌映射表或空结果说明。
- `brand=true && brandScopeCateNameTreeSet=空集` 时是删除同步：即使基础正文已有母子品牌内容，也必须
...
- 当前规则同步分支 `priceRules=false && brand=true` 时，只改写品牌相关内容，不得重建 Step3、
  `# 输出格式` 或其他非品牌业务规则；作用域为空时，允许且必须删除品牌比价项及其所有结构落点、母子品牌映射表、
  品牌章节中的映射引用和相邻的孤立分隔线。
- 当前规则同步分支仅在 `priceRules=false && brand=false` 时保持基础正文中的母子品牌映射表；
  `priceRules=true` 或格式重构硬分支必须按本轮真实品牌作用域重建，空集时删除旧母子品牌内容。
- 附录不能代替品牌的特殊规则或兜底。不能因为名称是"品牌"就自行添加母子品牌互认规则。

#### 4.3.2 材质
- 材质关系的查询、过滤和去重只按 4.1.2、4.1.4 执行；本节只规定候选正文落点。
- `materialGroupCount>0` 时，材质比价项章节在"匹配"之后额外增加独立条目
  `- **材质对照表**：见下方附录。`，并在 `## 映射表` 中输出
  `### 材质对照表（每条原始材质行内可互匹配，不同行不可）`；`materialGroupCount=0` 时两处都不得出现。
- `material=true && materialScopeCateNameTreeSet=空集` 时是删除同步：即使基础正文已有材质内容，也必须
  删除材质比价项、材质引用行、材质对照表、材质专属字段规则及删除后多余的分隔线；若映射表只剩母子品牌，
  保留 `## 映射表` 和母子品牌子表，删除两个子表之间原有的 `---`。
- `material=true && materialScopeCateNameTreeSet` 非空但 `materialGroupCount=0` 时也是映射删除同步：
  删除基础正文已有的全部材质引用行和材质对照表；若 `## 映射表` 只剩该材质表则删除整个章节，若仍有
  母子品牌表则保留母子品牌表并删除两个子表之间的孤立 `---`。真实材质比价项、其规则和材质专属字段规则保留。
...
- 当前规则同步分支仅在 `priceRules=false && material=false` 时保持基础正文中的材质引用行和材质对照表；
  `priceRules=true` 或格式重构硬分支必须按本轮真实材质作用域重建，空集时删除旧材质内容。

#### 4.3.3 货号
- 当前规则同步分支只有 `priceRules=true` 时才允许重建货号相关规则和 Step3；`priceRules=false`
  且只同步品牌/材质时不得改写货号规则、Step3 或输出结构。
- 仅当某个类目记录下该项真实 `compareLogic=货号匹配逻辑` 时，按上方枚举展开，并将该类目的
...
或未替换的示例类目。
这里比较的是 4.2 生成的归一化 `expectedPriority` 与最终 `expectedMatch`，不得比较原始 `infoSource`、
...
- 材质：标题固定 `### 材质对照表（每条原始材质行内可互匹配，不同行不可）`；严格按
  `materialRetentionLedger` 的原始响应相对顺序输出。第一行或当前行作用域集合与上一行不同时输出
  `#### 适用范围：<作用域标签>`；仅相邻且作用域集合完全相同的连续行共用标题，禁止全局聚合后重排。
  标题数必须精确等于 `materialScopeRunCount`，不得按 `materialGroupCount` 为每条材质原文重复标题。
  具体维度标签为 `<cateNameTree>｜<compareItem>`，多个标签用 `；` 连接；
  某 categoryId 下唯一材质比价项精确等于"材质"时只写 `<cateNameTree>`。最多 50 条去重原文，
  同一原文命中多个作用域仍只计 1 条、输出 1 次，实际不足时不补齐。

### 4.6 调用校验前的必检项

1. 从 4.5 格式模板中完整复制 `# 输出格式` 固定块至 `key_diff_point` 规则，不因长度或类目裁剪；
   保留 7 条通用规则，仅在真实比价项包含材质时追加材质专属规则。
2. 执行过 4.2 时，必须生成内部转换账本；命中受控别名时候选必须只包含标准枚举的完整展开，不得残留
...
6. 映射门禁：
   - 格式重构硬分支，或当前规则同步分支 `brand=true` 时：`brandScopeCateNameTreeSet` 非空则必须有本轮品牌关系查询与过滤记录；
     过滤后非空输出映射表，为空静默省略；品牌映射必须通过 4.1.3 的完整过滤、数量、格式和连续编号门禁。
   ...
   - 格式重构硬分支，或当前规则同步分支 `material=true` 时：`materialScopeCateNameTreeSet` 非空且
     ...
     必须至少命中一个真实作用域，并按 4.1.4 输出完整适用范围标题；标题数必须精确等于
     `materialScopeRunCount`，且按出现顺序提取的 `#### 适用范围` 标题序列不得存在两个相邻标题逐字符相同的情况，
     存在时必须删除后一个重复标题，
     且不得调整两标题之间及其后的材质原文顺序。调用校验前还必须按原始字符串扫描：
     每个 `#### 适用范围：` 前必须是文本起始或换行，标题末尾必须紧跟换行，不得与 `materialText` 出现在同一行；
     忽略中间材质行后单独提取标题序列，任意相邻两个标题逐字相同时仍必须合并。
     任一条件不满足不得调用校验；
     `material=true && materialScopeCateNameTreeSet` 非空但 `materialGroupCount=0` 时，候选必须删除基础正文
     已有的全部材质引用行和材质对照表，并清理空映射章节或孤立分隔线；候选不得残留旧材质映射，
     但必须保留当前规则仍存在的材质比价项和材质专属字段规则；
...
8. **当前规则同步分支专用硬校验**：
   ...
   - `material=true && materialScopeCateNameTreeSet=空集` 时，服务端 Diff 必须逐项体现基础正文已有材质内容的删除；
     若 Diff 未删除材质表、材质引用或材质专属字段规则，禁止形成提案。
   - `material=true && materialScopeCateNameTreeSet` 非空但 `materialGroupCount=0` 时，若基础正文存在材质引用行
     或材质对照表，服务端 Diff 必须明确体现这些旧映射内容及孤立分隔线的删除；Diff 未体现完整删除时禁止形成提案。
     本条件不得要求删除仍由当前类目规则定义的材质比价项或材质专属字段规则。
