...
进入 EDIT 后，在任何业务 MCP、候选生成或校验调用之前，必须完整读取本文件至末行。
若首次读取被截断，必须继续按行号或标题分段读取同一文件，至少逐字获取：

1. 第 1 节的全部分支判定和分支执行清单；
2. 第 4.5 节从“候选提示词格式模板”标题到该模板外层围栏结束的全部内容；
3. 第 4.6、4.7、4.8 节的全部门禁和校验规则；
4. 第 5 节的完整 EDIT 提案模板及输出硬校验。

只有上述内容全部实际读取后，才可记录 `editWorkflowFullyLoaded=true`；只有第 4.5 节模板
已读到完整外层围栏结束，才可记录 `editCandidateTemplateLoaded=true`。
任一值不为 `true` 时，禁止调用业务 MCP、生成候选或调用 `tool_validate_prompt_skeleton`；
只能继续读取本文件，读取确实失败时只返回真实的 Skill 文件加载错误。

禁止根据校验工具 `errors`、旧 Prompt、历史候选、模型记忆或通用 Prompt 知识反推、补写或仿造第 4.5 节格式。
禁止生成“精简但完整”“大致结构”“代表性材质”、省略号、`...`、`同上`、`选取相关`或任何占位式候选；
数量只能按本文的真实过滤账本和上限规则确定，不得为节省 token 自行抽样。
...
执行本节前必须已有 `editWorkflowFullyLoaded=true && editCandidateTemplateLoaded=true`。
候选的第一个字符必须是下方模板围栏内 `# 角色` 的 `#`，最后一个字符必须属于固定 `# 输出格式` 块的最后一行；
禁止在围栏内模板之前或之后拼接自定义草稿、格式说明或推理文字。
