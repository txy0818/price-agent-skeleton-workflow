...
本文件包含 EDIT 流程的完整执行规范，禁止读取历史完整提示词或示例文件作为生成范式。
`上下文.promptVersionId>0` 时读取该流程文件。
...
2. **当前规则全量同步分支**：用户要求"根据/按照/通过当前或最新的比价项规则、母子品牌关系、材质关系更新/同步提示词"或等价表达。
   触发判别：上述词必须作为**数据来源/依据**出现（如"根据母子品牌关系同步""按最新规则更新"），而非作为修改目标（如"新增一个母子品牌""删除这组品牌"属于精确修改直达分支）。
   锁定 `editTargetScope={priceRules:true, brand:按真实比价项门禁, material:按真实比价项门禁}`；
   必须完整执行规则查询并逐项重建核对全部比价项、品牌映射和材质映射。
...
> **"优化提示词"单独出现时走分支 4；"根据规则/母子品牌/材质优化提示词"走分支 2。**
...
### 3.2 compareItem 集合与映射门禁

以全部有效 `ruleTableInfo[].compareItem` 建立集合：

- 集合中**精确包含"品牌"**时：必须调用 `radar_query_brand_relation`（见 3.3）；
  不包含"品牌"时禁止调用，对应映射表不得生成。
- 集合中任一 `compareItem` **包含"材质"**时：必须调用 `query_material_leaf`（见 3.4）；
  无任何 `compareItem` 包含"材质"时禁止调用，对应映射表不得生成。
...
2. 以规则响应中的 `cateName` 为主要过滤依据（辅以 `cateNameTree`、`belongBusiness`），
   对全部有效组做相关性过滤；多类目分别过滤后取并集，不得扩大成"电商全站"；
   模型常识只能用于筛除不相关项，不得据此新增、恢复、合并、改名或补别名；
...
扩大作用域的措辞。**100 组是上限，不是生成目标**。

用户明确指定某组的新增、删除、替换或成员调整属于精确修改，直接执行用户指令，不查询关系库复核。

### 3.4 同款材质关系

本轮至少一个有效 `compareItem` 包含"材质"时必须执行本节；否则不得执行（双向硬门禁）。
...
处理步骤：
1. **类目过滤**：以本轮 `cateName`（辅以 `cateNameTree`、`belongBusiness`）为依据，
   只保留当前类目直接使用的材料，明显无关或不确定的行删除；多类目分别过滤后取并集，
   不得扩大成"电商全站"；模型常识只能用于筛除不相关项，不得据此新增或改写内容。
...
|---|---|---|---|---|
| 精确修改直达 | 跳过 | 跳过 | 跳过，在原文上局部修改 | 仅执行第 1、3、4、5、8 条 |
| 当前规则全量同步 | 必须 | 必须 | 必须完整套用 | 全部 |
| 格式重构硬分支 | 跳过 | 必须 | 必须完整套用 | 全部 |
| 无方向普通修改 | 跳过 | 跳过 | 跳过，在原文上修格式/歧义 | 仅执行第 1、3、4 条 |

### 4.1 EDIT 目标覆盖账本（当前规则全量同步分支专用）

当前规则全量同步分支必须在内部建立一张**比价项覆盖核对表**（禁止将其内容复制到 `promptContent`）。
对每个真实 `ruleTableInfo[]` 依次记录：
...
**受控别名**：
- `通用匹配逻辑`：左右两侧该比价项一致，或不冲突，或有交集，即可匹配。
- `优先完美匹配逻辑`：左右两侧该比价项一致或不冲突即可匹配。
- `影响价格匹配逻辑`：左右两侧该比价项一致或不冲突即可匹配。
- `严格匹配逻辑`：左侧有该比价项时，右侧竞品必须有且一致才可匹配；左侧有、右侧无时不可匹配；左侧无、右侧有时可匹配。
- `货号匹配逻辑`：
...
**品牌**：仅当真实比价项集合包含"品牌"时，按 3.3 查询母子品牌关系；无"品牌"时禁止查询和输出。
...
### 4.3 候选提示词格式模板（当前规则全量同步 + 格式重构硬分支）
...
7. **当前规则全量同步分支专用硬校验**：
   - 比价项覆盖核对表中真实比价项数量、名称、顺序与 `ruleTableInfo[]` 完全一致；
   - 候选全文通过全部硬校验；
   - 用户请求的每个目标均记录为"已修改""已核对且一致"或"按真实比价项门禁不适用"，不得静默忽略；
   - 若服务端 Diff 只包含映射变化，则必须已逐项证明所有比价项规则完全一致；否则禁止形成提案。
   - 任一项不满足都不得调用校验，不得形成只修改映射表的不完整提案。
8. 修改涉及母子品牌表时，完整执行格式、过滤、数量和机械重编号校验；任一条件不满足禁止调用校验。
...
- `respCode=1 && data.valid=true && data.diffRecordId>0`：成功，锁定本次 `promptContent`，保留 `diffRecordId`、`diffContent`。
  `data.diffContent` 在 EDIT 中不允许为空。
- `respCode=1 && data.valid=true && data.diffRecordId<=0`：立即异常结束，回复
...
用户紧邻有效提案回复"展开完整提示词"时，只执行本分支。上一条必须来自本轮真实 `tool_validate_prompt_skeleton` 响应，且
...
若上一条 assistant 可见消息包含以下三项，模型侧门禁通过，必须使用该非零 ID 调用写入工具：
1. `## 提示词修改提案`
2. 非零修改建议 ID（`diff_id`）
3. 明确确认话术

进入后禁止输出任何自然语言或执行其他工具，必须先调用写入工具。
...
- `promptVersionId`：从当前确认回合业务上下文读取 `promptVersionId`，记为 `confirmationPromptVersionId`。
  原始值为非负整数时直接使用；字段未注入、为空或为占位符时统一令其为 0。
  不得从提案"基础提示词 ID"、历史消息或工具结果补值。
  只要存在有效非零 `promptDiffRecordId`，就不得因 `promptVersionId` 缺失而阻止写入。
  仅当上下文显式提供负数或非数字值时停止，只回复："当前确认回合提供的 promptVersionId 非法，未执行提示词写入。"
- `sourceType=2`（模型优化）
...
`data.promptExists=true` 时状态改为"已存在，未新建"。不得总结变更内容；返回的新 ID 仅展示，不更新上下文。
