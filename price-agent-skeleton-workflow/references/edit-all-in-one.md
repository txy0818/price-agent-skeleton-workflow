...
- **当前规则同步：`priceRules=true`**
  ...
  - 追加执行：`brand=true` 时追加 4.1.3 主子品牌关系查询、4.3.1 品牌；`material=true` 时追加 4.1.4 同款材质关系查询、4.3.2 材质。
...
- **当前规则同步：`priceRules=false && brand=true && material=false`**
  - 必须执行：4.1.1 类目规则查询；4.1.2 统一作用域过滤；4.1.3 主子品牌关系查询；4.3.1 品牌；4.4 映射与比价项编号规则。
...
- **当前规则同步：`priceRules=false && brand=false && material=true`**
  - 必须执行：4.1.1 类目规则查询；4.1.2 统一作用域过滤；4.1.4 同款材质关系查询；4.3.2 材质；4.4 映射与比价项编号规则。
  - 禁止执行：4.2 比价项转换账本；4.1.3 主子品牌关系查询；4.3.1 品牌；4.3.3 货号；4.5 候选提示词格式模板。
  - 4.6 调用校验前的必检项：第 4、5、6、7 条。

- **当前规则同步：`priceRules=false && brand=true && material=true`**
  - 必须执行：4.1.1 类目规则查询；4.1.2 统一作用域过滤；4.1.3 主子品牌关系查询；4.1.4 同款材质关系查询；4.3.1 品牌；4.3.2 材质；4.4 映射与比价项编号规则。
...
- `brand=true`：必须先查类目规则，用真实比价项判断是否允许品牌映射，再查询品牌关系。
- `material=true`：必须先查类目规则，用真实比价项判断是否允许材质映射，再查询材质关系。
...
历史响应、基础正文、常识或用户未绑定到真实比价项的数据反推作用域。若全部真实 `compareItem` 均不包含"材质"，
...
`brand=true` 时只允许改写母子品牌映射表，`material=true` 时只允许改写材质引用行和材质对照表；
...
- 当前规则同步分支 `priceRules=false && brand=true` 时，只替换母子品牌映射表子表；不得重建
  `## 比价项`、Step3、`# 输出格式` 或其他业务规则章节。
- `brand=false` 时候选必须保持基础正文中的母子品牌映射表，不得顺手改写。
...
- 当前规则同步分支 `priceRules=false && material=true` 时，只替换材质引用行和材质对照表子表；
  不得重建 `## 比价项`、Step3、`# 输出格式` 或其他业务规则章节。
...
5. **材质零作用域门禁**：执行 `priceRules=true` 或格式重构硬分支时，若真实 `compareItem` 均不含"材质"，
   必须同时满足 `materialScopeCateNameTreeSet=空集`、`materialGroupCount=0`、无材质工具调用，且重建后的候选
   不含 `材质对照表`、`材质value` 或任何材质专属说明；任一不满足均禁止调用校验。
