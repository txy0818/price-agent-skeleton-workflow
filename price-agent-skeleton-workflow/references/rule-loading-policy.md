...
3. 只有当前 MCP 清单存在对应关系工具且流程确有必要时才调用；否则停止需要推导的分支。
...
仅在当前 MCP 清单确实存在主子品牌关系查询工具、且流程需要模型自行生成或判断映射时调用。
用户明确要求新增、删除或替换某组关系时不进入本节，也不查询关系库复核。
...
| 流程 | 类目规则 | 品牌 | 材质 |
|---|---|---|---|
| 初始化 | 从真实上下文取得类目后逐个查询 | 工具存在时查询并过滤 | 工具存在时查询并过滤 |
