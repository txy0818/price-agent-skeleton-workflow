...
4. 先以全部有效 `ruleTableInfo[].compareItem` 建立集合：仅集合中精确存在“品牌”时调用
   `radar_query_brand_relation`；仅精确存在“材质”时加载材质关系 Skill 并调用其规定的 MCP。
   未命中的关系工具不得调用，对应映射表不得生成。应调用的工具不可用、失败或响应无法解析时
   停止，禁止用模型知识兜底。
...
仅当本轮至少一个有效 `compareItem` 精确等于“品牌”时执行本节；否则跳过查询并禁止输出母子品牌
映射表。不得根据特殊规则、基础提示词旧内容或模型常识绕过此门禁。
...
仅当本轮至少一个有效 `compareItem` 精确等于“材质”时执行本节；否则跳过查询并禁止输出材质
对照表。不得根据特殊规则、基础提示词旧内容或模型常识绕过此门禁。
