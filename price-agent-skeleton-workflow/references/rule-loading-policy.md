...
过滤只决定是否内嵌，不得改变返回的主子关系。初始化最多保留 **50 个主品牌组**，一个
`mainBrand + subBrands[]` 算一组；不足 50 组不得用无关品牌补齐。超过时依次优先：响应中的
类目精确匹配、业务线精确匹配、品牌主营商品与当前类目直接相关；仍无法区分时保留原顺序。

生成后必须二次审计映射名称与全部当前作用域。例如作用域仅为美妆、美妆工具、香水时，只保留
明显经营化妆品、个护或香水的品牌组；食品、酒水、家电、数码、汽车、服饰、家纺、母婴、
...
材质只保留当前作用域必要映射，并分别受 50 组上限约束。过长时精简行文或改成运行时查询
指令，不得硬截断规则。
