...
`materialScopeLedger` 账本行。接口返回条数也不是本轮命中证据。无法根据“当前原文 + 当前账本行的
`cateNameTree + compareItem`”明确建立归属时，该账本行固定判为不命中；禁止把不确定改判为全部作用域命中。
...
- 最多输出作用域过滤且去重后的前 80 条原始材质行；未命中行不占名额，同一行命中多个作用域仍只计
  1 条、输出 1 次，实际不足时不补齐。
...
6. 映射门禁：
   ...
   - `materialScopeCateNameTreeSet` 非空且 `materialOutputGroups` 非空时：对应比价项章节必须存在固定行
     ...
     `data.materialText[]`，且必须先明确命中 key 中列出的全部真实作用域；所有 value 合计不超过 80 条
     过滤且去重后的原文，未命中行不得占用 80 条名额。
     任一条件不满足不得调用校验。
