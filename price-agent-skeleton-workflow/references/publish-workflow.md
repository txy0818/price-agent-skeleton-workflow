...
保存、验证、发布相互独立；Agent 不自动运行验证。所有 MCP 结果按 [SKILL.md](../SKILL.md) 的失败与重试
...
- 已有线上版本：展示「线上→待发布」的 Diff，**不展开待发布全文**；同时给出两个任务 ID、
  ...
  本流程不调用 `tool_validate_prompt_skeleton`，因此没有服务端计算的 `data.diff_content`；
  该 Diff 由模型基于两次查询取回的完整内容自行比对得出，只允许如实反映两份内容的差异。
