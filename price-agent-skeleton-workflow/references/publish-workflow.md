...
2. 调用
   `tool_query_prompt_skeleton(rule_group_id=当前业务上下文.ruleGroupId, prompt_version_id=<待发布提示词ID；未提供则0>, version_name=<待发布提示词名称；未提供则空>, query_online=false, operator=当前业务上下文.operator)`，
...
2. 使用重新查询得到的两个提示词 ID 再执行 `compare_prompt_validation`，确认门禁仍通过。
3. 调用一次
