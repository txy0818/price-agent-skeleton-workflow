...
3. 按 `[B4]`～`[B6]` 对本批每条样本建立证据并选择唯一一级分类，再统计三类和证据不足数量；不得
   先按批次整体分类。
...
5. 用户要求“整合修改建议”时不查询下一页；合并同一续批锁下已完成的阶段结论并按下方“整合修改
   建议”执行。仅存在“是否修改提示词=是”的第 2、3 类样本时生成一份完整候选提示词并执行一次 `[S3]`。
...
- 待核验（证据不足）：<数；列出缺失的 SPU 口径判定证据并补证后重跑>
### 明细
| Badcase ID | 人工/模型标签 | 一级分类 | SKU/SPU 口径与关键证据（快照事实 vs extracted） | 具体原因与建议 |
|---|---|---|---|---|
| `<case_id>` | <标签> | <三类之一；未通过门禁时写对应“待核验”> | <比价项：快照事实 / 左右 value / match> | <具体原因与建议> |
### 本批候选修改
- 缺陷与样本：<仅“是否修改提示词=是”的第 2、3 类样本及 case ID>
- 候选建议：<待最终整合的规则修改；无缺陷写“本批无提示词修改建议”>
- 不写入提示词：<合理粒度差异、两种待核验结果，以及原因不在提示词的真实 Badcase>
...
5. 三类数量与“待核验（标签不确定）”“待核验（证据不足）”数量之和必须等于本批去重后的明细数；
...
2. 合并指向同一规则的重复建议；结合商品快照证据、`extracted` 抽取结果、比价规则和映射
   处理冲突。合理粒度差异、两种待核验结果及具体原因不在提示词的真实 Badcase 不得写入提示词。
3. 若合并后没有“是否修改提示词=是”的样本，按统一流程 `[B7]` 不执行 `[S3]`，只能使用下方
   “无修改整合结果模板”。若存在满足门禁的样本，将同一续批锁下的缺陷记录合并后交给 `[B7]`，只生成一份候选完整
...
- 不修改部分：<明确排除的合理粒度差异、两种待核验结果和原因不在提示词的真实 Badcase>
### Diff
```diff
