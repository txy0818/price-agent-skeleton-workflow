...
3. 调用
   `ToolQueryValidationResult(validation_task_id=<任务ID>, label_filter=3, page_size=20, operator=当前业务上下文.operator)`。
...
`ToolEditPromptSkeleton`。后续批次必须合并当前会话中同一任务、同一基础提示词的既有
