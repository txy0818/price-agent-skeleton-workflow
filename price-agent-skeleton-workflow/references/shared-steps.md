...
5. INITIALIZE、格式重构及 EDIT 当前规则全量同步分支必须通过映射门禁：`compareItem` 有“品牌”时
   ...
   非精确分支输出材质表前还必须建立内部材质分组账本：每组记录真实父节点 ID/名称、仅供校验的
   祖先路径、每个成员 ID/名称/`isLeaf`/`parentId`；只接受至少两个
   `isLeaf=true && parentId=该组父节点ID` 的直接末端叶子。候选组名必须逐字符等于父节点名称，成员
   必须与该父节点的直接叶子集合逐项一致；祖先路径不得写入组名。不同父节点合并、父节点充当成员、
   单叶子组、凭名称相似归组，或使用 `·`、`/`、`>`、`-` 拼接祖先名称时禁止 S3。
