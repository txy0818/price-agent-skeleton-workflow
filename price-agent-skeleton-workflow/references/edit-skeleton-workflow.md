...
> **精确修改直达分支**：用户给出可唯一定位的增删改时，将其作为已授权修改目标，不执行 `[S2]`，不调用关系、规则或验证任务 MCP 复核；单个母子品牌或材质关系的新增、删除、替换及成员调整均属于此分支。正文格式合规时只做该修改及其必需的机械一致性调整；不合规时同时按 `skeleton-format.md` 修复。删除带序号的母子品牌组或比价项后，必须把同一序列的后续条目从删除位置起连续重新编号，并同步更新数量标题；重新编号属于本次删除的必要组成，不算扩大修改范围。用户明确新增母子品牌或材质关系时直接加入，即使超过 100 个品牌组或 50 个材质组也不得拒绝、裁剪或删除其他关系；同步更新实际数量。`writeMode` 始终为 `EDIT`。
