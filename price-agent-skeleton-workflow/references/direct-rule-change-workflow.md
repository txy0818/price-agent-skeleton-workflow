...
4. 不调用验证任务或验证结果查询。调用
   ...
   只取 `data.data_table_json`。对照这些规则、映射与目标提示词判断用户要求是否合理。
...
调用一次
`ToolEditPromptSkeleton(rule_group_id=<规则组ID>, prompt_version_id=<基础提示词ID>, prompt_content=<已确认的完整内容>, change_reason=<规则修改原因>, diff_content=<已展示Diff>, source_type=2, operator=当前业务上下文.operator)`。
