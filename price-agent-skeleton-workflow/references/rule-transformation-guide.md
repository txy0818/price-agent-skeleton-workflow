# 规则转换指南

用于从 MCP 返回的规则 JSON 生成同款判定提示词。这里只规定转换方法，不提供当前业务事实。

## 权威输入

只使用以下业务内容：

- `radar_query_price_rule.data.labelCateRule`：类目、业务线、搜索方式、比价项、信息来源和匹配逻辑。
- 当前 MCP 清单中实际存在的主子品牌关系查询结果：经可信关键词和当前作用域约束后的关系。
- `radar_query_material_relation`：按当前 `cateName`、`belongBusiness` 过滤后的同款材质关系。

JSON 缺失、为空或无法解析时停止，不使用参考示例补齐。本指南不用于用户已明确指定具体文本的
精确修改直达分支。

## 转换步骤

1. 从 `labelCateRule.ruleTableInfo.ruleTableInfo[]` 提取当前类目的比价项列表。一个 `compareItem` 对应提示词中的一个比价项章节，以及输出 JSON `extracted` 中的一个字段。
2. 将 `infoSource` 转成“抽取优先级”。保留原始 `>`、`=`、`/`、左右侧差异和兜底条件，不自行简化或改序。
3. 将 `compareLogic` 转成“匹配规则”。只有输入 JSON 或已查询数据提供具体定义时才展开；只有逻辑名称时不得凭参考示例补写细则。
4. 将响应中明确返回的特殊条件和适用范围放入总原则或对应比价项的“例外和边界”；未返回时不得补写。
5. 主子品牌与材质关系只使用按 [rule-loading-policy.md](rule-loading-policy.md) 过滤后的结果。
   初始化时允许结合 `cateName + belongBusiness` 与业务常识排除明显无关候选；映射较长时
   保留当前类目必要且较常见的分组，其余写成运行时查询指令，禁止复制全量候选。模型自行生成时
   母子品牌最多 100 个主品牌组，材质最多 50 个材质组；不足时不补齐。用户明确要求新增的
   母子品牌关系直接加入，不受 100 组限制。母子品牌逐组写成
   `序号. 母品牌→子品牌1|子品牌2`，不得加粗，不得使用无序列表或冒号格式。
6. 按 [skeleton-format.md](skeleton-format.md) 生成角色、输入说明、执行步骤、判定规则、映射表、图片说明和 JSON 输出格式。

## 简短示例

输入：

```json
{
  "compareItem": "品牌",
  "infoSource": "sku图品牌=商品属性>商品标题",
  "compareLogic": "通用匹配逻辑"
}
```

转换：

```markdown
### 品牌

- **抽取优先级**：SKU 图品牌与商品属性同级，高于商品标题。
- **匹配规则**：<仅使用本轮数据中“通用匹配逻辑”的具体定义>
```

特殊规则输入：

```text
儿童版与成人版不可相互匹配。
```

转换到对应比价项：

```markdown
- **例外和边界**：儿童版与成人版不匹配。
```

不得因此额外生成输入中没有的“系列”“抗菌等级”等比价项。

## 完成检查

- 每个有效 `compareItem` 是否恰好生成一个章节和一个 `extracted` 字段？
- 是否遗漏或改变 `infoSource` 的优先级与左右侧差异？
- 是否把逻辑名称误写成未经查询的详细规则？
- 是否完整保留适用的特殊规则、条件和例外？
- 是否只使用关系工具真实返回且通过当前类目、业务线过滤的映射？
- 模型自行生成的母子品牌是否不超过 100 组、材质是否不超过 50 组，且没有为凑数量保留低相关、不常见或跨行业项？用户明确新增的母子品牌是否已直接加入且未受数量限制？
- 母子品牌是否使用连续序号、`→` 和 `|`，且没有加粗、无序列表或冒号？
- 是否加入了输入 JSON 没有的比价项、阈值或业务事实？
